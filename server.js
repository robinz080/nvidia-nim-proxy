...
// Diagnostic: check which mapped models are currently valid on NVIDIA's side
app.get('/v1/models/check', async (req, res) => {
  const results = {};
  for (const [alias, nimModel] of Object.entries(MODEL_MAPPING)) {
    try {
      ...
      results[alias] = { model: nimModel, status: 'OK' };
    } catch (e) {
      results[alias] = { model: nimModel, status: e.response?.status || e.message };
    }
  }
  res.json(results);
});
